            ...
            if (event.key === 'Enter') {
                ...
                if (currentWordIndex < words.length) {
                    ...
                    if (wrongAnswers.length === 0) {
                        ...
                        startAgainMistButton.textContent = 'Start Mistakes';
                        startAgainButton.textContent = 'Start Again';

                        result.appendChild(startAgainMistButton);
                        result.appendChild(startAgainButton);
...
function startGameWithExistingMistakes () {
    ...
    console.log(words);
     ...
     words = wrongAnswers.map(({correctAnsw}) => ({
        translation: words.find(word => word.word.toLowerCase() === correctAnsw.toLowerCase()).translation,
        word: correctAnsw || ''
    }))
