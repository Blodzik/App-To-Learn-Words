        ...
        checkButton.addEventListener('click', () => {
            ...
            if (currentWordIndex < words.length) {
                ...
                if (wrongAnswers.length === 0) {
                    ...
                    const startAgainButton = document.createElement('button');
                    const startAgainMistButton = document.createElement('button');
                    const startAgainDivM = document.createElement('div');

                    startAgainMistButton.classList.add('start-again-mist-button');
                    startAgainButton.classList.add('start-again-button-mist');
                    startAgainDivM.classList.add('start-again-div-m');

                    startAgainMistButton.textContent = 'Start With Mistakes';
                    startAgainButton.textContent = 'Start Again';

                    startAgainDivM.appendChild(startAgainButton);
                    startAgainDivM.appendChild(startAgainMistButton);

                    result.appendChild(startAgainDivM);

                    startAgainButton.addEventListener('click', () => {
                        startGameWithExistingWords();
                    });

                    startAgainMistButton.addEventListener('click', () => {
                        startGameWithExistingMistakes();
                    });
                }
            }
        })
...
function startGameWithExistingMistakes () {
    loadMistakesFromLocalStorage();
    console.log(words);

    words = wrongAnswers.map(({userAnsw, correctAnsw}) => ({
        translation: userAnsw,
        word: correctAnsw
    }));
